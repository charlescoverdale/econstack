...
**Only stop to ask the user when:** document type is unrecognised, or fix confirmation is needed (--fix mode).
**Never stop to ask about:** which checks to run (run all applicable), grading methodology, or output filename.
...
- `--full` : Skip interactive menus, output full audit report with all categories
