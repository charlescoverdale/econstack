...
**Only stop to ask the user when:** local authority name is ambiguous (multiple matches), or the user requests a comparison LA.
**Never stop to ask about:** which sections to include when --full is specified, data year (use latest available), or benchmark selection (default to the LA's own country average).
...
- Be specific about dates. "[Year]" not "recent". State the data year for every metric.
- The key stats block at the top is always included, even for partial outputs.
- The companion JSON must include all data points, source metadata, and comparison benchmarks.
- **Always include a data sources section** listing the specific ONS datasets used:
  - ONS Annual Population Survey (employment, economic activity)
  - ONS Annual Survey of Hours and Earnings (workplace-based earnings)
  - ONS Business Register and Employment Survey (industry employment, LQs)
  - ONS UK House Price Index or Land Registry (house prices)
  - ONS Mid-Year Population Estimates (population)
  - ONS Regional GVA (GVA per job, productivity)
  - MHCLG Index of Multiple Deprivation (deprivation, England only)
  - NOMIS Labour Market Statistics (claimant count, JSA/UC)
  - DfE Skills data (NVQ qualifications)
  Include the data vintage (year) for each dataset used.
