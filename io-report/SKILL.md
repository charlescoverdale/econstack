...
- Location keywords: London, Manchester, Birmingham, Glasgow, Edinburgh, Leeds, Bristol, Liverpool, Cardiff, Belfast -> uk
- **Ambiguous locations** (exist in both countries): Newcastle, Richmond. If the user says just "Newcastle" without further context, ask: "Did you mean Newcastle upon Tyne (UK) or Newcastle and Lake Macquarie (Australia)?" Same for Richmond.
- If ambiguous for other reasons: ask via AskUserQuestion
...
**Australian computation notes (country = au):**

When computing for an AU SA4, map the SA4 JSON field names to the computation formulas:
- `employment_per_AUDm` in the SA4 JSON maps to `directEmploymentPerMillion` in the formulas
- `output_multiplier_type1` maps to `outputMultiplier`; `output_multiplier_type2` maps to `outputMultiplierTypeII`
- `employment_multiplier_type1` maps to `employmentMultiplier`

**AU data limitations:** The SA4 JSON files do NOT contain `gvaToOutputRatio` or `averageEarningsPerJob`. For AU reports:
- **GVA impact**: Omit from the report, or use a rough national approximation (GVA/output ratio ~0.55 for Australia, from ABS national accounts). If using the approximation, note: "GVA estimated using national average GVA-to-output ratio. Regional ratio may differ."
- **Earnings impact**: Omit from the report. State: "Earnings impact not computed. SA4-level earnings by industry data is not available from ABS."
...
Conservative:  deadweight=35%, displacement=35%, leakage=25%, substitution=5%  (net factor: 0.285)
...
**Division S (Other services) handling:** If the user requests sector S and country = au, respond: "The ABS 2023-24 IO tables do not separately identify Other Services (ANZSIC Division S). This sector is folded into adjacent divisions. Try a more specific sector: Arts & recreation (R), Administrative & support (N), or another ANZSIC division."
